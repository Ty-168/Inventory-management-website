...
export const FloatingNav = ({
  ...
  const [visible, setVisible] = useState(true); // Default visibility

  useMotionValueEvent(scrollYProgress, "change", (current) => {
    // Ensure `current` is defined and is a number
    if (typeof current === "number") {
      let prevScroll = scrollYProgress.getPrevious()!;
      let direction = current - prevScroll;

      // Show navbar ONLY when scrolling down
      if (direction > 0) {
        setVisible(true); // Hide when scrolling down
      } else {
        setVisible(false);  // Keep it visible when idle or scrolling up
      }
    }
  });
